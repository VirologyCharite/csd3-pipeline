...
function panel()
{
    echo "  alignment-panel-civ.py DNA large started at $(date)" >> $log
    ...
    do
        ...
        JSON=../03-diamond-civ-dna-large/$task.json.bz2
        test -f $JSON || {
...
then
    ...
    if [ $SP_SKIP = "1" ]
    then
        echo "  Panel civ DNA large encephalitis is being skipped on this run." >> $log
        skip
    elif [ -f $out ]
    then
        if [ $SP_FORCE = "1" ]
