...
function panel()
{
    echo "  alignment-panel-civ.py DNA started at $(date)" >> $log
    ...
    echo "  alignment-panel-civ.py DNA stopped at $(date)" >> $log

    echo "  proteins-to-pathogens-civ.py DNA started at $(date)" >> $log
    echo summary-proteins | \
        proteins-to-pathogens-civ.py \
            --proteinGenomeDatabase $dnaProteinGenomeDB \
            --taxonomyDatabase $taxonomyDB \
            > $out
    echo "  proteins-to-pathogens-civ.py DNA stopped at $(date)" >> $log
}
...
then
    ...
    if [ $SP_SKIP = "1" ]
    then
        echo "  Panel civ DNA encephalitis is being skipped on this run." >> $log
        skip
    elif [ -f $out ]
    then
        if [ $SP_FORCE = "1" ]
