...
function panel()
{
    echo "  alignment-panel-civ.py RNA started at $(date)" >> $log
    ...
    echo "  alignment-panel-civ.py RNA stopped at $(date)" >> $log

    echo "  proteins-to-pathogens-civ.py RNA started at $(date)" >> $log
    echo summary-proteins | \
        proteins-to-pathogens-civ.py \
            --proteinGenomeDatabase $rnaProteinGenomeDB \
            --taxonomyDatabase $taxonomyDB \
            > $out
    echo "  proteins-to-pathogens-civ.py RNA stopped at $(date)" >> $log
}
...
then
    ...
    if [ $SP_SKIP = "1" ]
    then
        echo "  Panel civ RNA encephalitis is being skipped on this run." >> $log
        skip
    elif [ -f $out ]
    then
        if [ $SP_FORCE = "1" ]
