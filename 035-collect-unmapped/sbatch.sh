...
# NOTE!! The following must have the identical value set and used in
# collect-unmapped.sh
out=$task.fastq.gz
...
then
    echo "  This is not a sample or run for which we are collecting unmapped reads. Taking no action." >> $log
    schedule=0
elif [ -f $out ]
then
